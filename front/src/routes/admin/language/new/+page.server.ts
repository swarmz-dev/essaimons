...
export const actions: Actions = {
    default: async (event: RequestEvent): Promise<void> => {
        ...
        try {
            const { data: returnedData } = await locals.client.post('api/admin/language/create', formData, {
                headers: {
        ...
        if (isSuccess) {
            redirect(
                303,
                `/${cookies.get('PARAGLIDE_LOCALE')}/admin/language/edit/${data.language.code}`,
                {
                    type: 'success',
