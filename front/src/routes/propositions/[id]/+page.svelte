<script lang="ts">
    ...
    import PropositionOverviewTab from '#lib/partials/proposition/detail/tabs/PropositionOverviewTab.svelte';
    import PropositionClarificationsTab from '#lib/partials/proposition/detail/tabs/PropositionClarificationsTab.svelte';
    import PropositionAmendmentsTab from '#lib/partials/proposition/detail/tabs/PropositionAmendmentsTab.svelte';
    import PropositionVoteTab from '#lib/partials/proposition/detail/tabs/PropositionVoteTab.svelte';
    import PropositionMandatesTab from '#lib/partials/proposition/detail/tabs/PropositionMandatesTab.svelte';
    import PropositionFollowupTab from '#lib/partials/proposition/detail/tabs/PropositionFollowupTab.svelte';
...
<div class="proposition-detail space-y-6 lg:space-y-8">
    ...
    {#if activeTab === 'overview'}
        <PropositionOverviewTab {proposition} {canCommentOnSection} {openCommentForSection} {formatFileSize} />
    {:else if activeTab === 'clarifications'}
        <PropositionClarificationsTab
            comments={commentsByScope.clarification}
            {canCommentClarification}
            onAddComment={() => (isClarificationDialogOpen = true)}
            onReplyComment={openClarificationReply}
            onEditComment={openClarificationEdit}
            onDeleteComment={openClarificationDelete}
            onReportComment={openReportDialog}
            {formatDateTime}
            {getSectionLabel}
        />
    {:else if activeTab === 'amendments'}
        <PropositionAmendmentsTab
            {events}
            comments={commentsByScope.amendment}
            {canManageEvents}
            {canCommentAmendment}
            onAddEvent={openEventCreate}
            onViewEvent={openEventDetail}
            onEditEvent={openEventEdit}
            onDeleteEvent={confirmEventDelete}
            {isEventEditableByCurrentUser}
            onAddAmendmentComment={() => (isAmendmentDialogOpen = true)}
            onReplyAmendmentComment={openAmendmentReply}
            onEditAmendmentComment={openAmendmentEdit}
            onDeleteAmendmentComment={openAmendmentDelete}
            onReportComment={openReportDialog}
            {formatDateTime}
            {translateEventType}
            {getSectionLabel}
    ...
    {:else if activeTab === 'mandates'}
        <PropositionMandatesTab
            {mandates}
            {user}
            {canManageMandates}
            {canEvaluateDeliverable}
            onAddMandate={openMandateCreate}
            onEditMandate={openEditMandate}
            onDeleteMandate={confirmMandateDelete}
            onApplyForMandate={openApplicationDialog}
            onAcceptApplication={confirmAcceptApplication}
    ...
    {:else if activeTab === 'followup'}
        <PropositionFollowupTab {rolePermissionMatrix} {workflowAutomation} comments={commentsByScope.evaluation} {formatDateTime} {getSectionLabel} />
    {/if}
</div>
