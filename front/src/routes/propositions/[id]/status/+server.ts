...
export const POST: RequestHandler = async ({ params, request, locals }) => {
    ...
    try {
        ...
        const response = await locals.client.post(`/api/propositions/${encodeURIComponent(propositionId)}/status`, payload);
        return json({ isSuccess: true, data: response.data.proposition });
    } catch (error: any) {
